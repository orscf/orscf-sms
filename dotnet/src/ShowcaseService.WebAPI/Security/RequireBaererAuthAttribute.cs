  ...
  public class RequireBaererAuthAttribute : Attribute, IAsyncActionFilter {
    ...
    public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next) {
      ...
      try {
        ...
        context.Result = new ContentResult() {
          StatusCode = 401,
          Content = "'Authorization'-Header contains an invalid bearer token (decode failure)!"
        };
        return;
      }
      ...
      if (AccessSettings.Current.JwtAllowedIssuers != null && !AccessSettings.Current.JwtAllowedIssuers.Contains("*")) {
        if (!AccessSettings.Current.JwtAllowedIssuers.Contains(bearerContent.iss)) {
          context.Result = new ContentResult() {
            StatusCode = 401,
            Content = "'Authorization'-Header contains an invalid bearer token (invalid issuer)!"
          };
          return;
        }
      }
      ...
      if (DateTime.UtcNow > expirationTimeUtc) {
        context.Result = new ContentResult() {
          StatusCode = 401,
          Content = "'Authorization'-Header contains an invalid bearer token (expired)!"
        };
        return;
      }
